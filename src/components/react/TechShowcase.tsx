...
import { Check, Circle, Plus, Trash2, Zap, Palette, Layers, Database } from 'lucide-react'
...
function TaskManager() {
  const { tasks, filter, addTask, toggleTask, removeTask, setFilter, getFilteredTasks, getStats } = useTaskStore()
      ...
      <CardContent className="space-y-4">
        ...
        <Tabs value={filter} onValueChange={(v) => setFilter(v as typeof filter)}>
          <TabsList className="w-full">
            ...
            <TabsTrigger value="active" className="flex-1">进行中</TabsTrigger>
        ...
        <div className="space-y-2">
          ...
          {filteredTasks.length === 0 && (
            <p className="text-center text-sm text-muted-foreground py-4">暂无任务</p>
          )}
        </div>
      </CardContent>
    </Card>
    ...
    <Card className="border-border/50 shadow-lg">
      ...
      <CardContent>
        <div className="flex flex-wrap gap-3">
          ...
          <Badge className="bg-blue-500/10 text-blue-500 border-blue-500/20">自定义</Badge>
          <Badge className="bg-purple-500/10 text-purple-500 border-purple-500/20">主题</Badge>
          <Badge className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20">颜色</Badge>
        </div>
      </CardContent>
    </Card>
  )
      ...
      <CardContent>
        <Accordion type="single" collapsible className="w-full">
          <AccordionItem value="item-1">
            ...
            <AccordionContent>
              shadcn/ui 是一个基于 Radix UI 和 Tailwind CSS 构建的可复用组件集合。
              它不是一个传统的组件库——你不需要通过 npm 安装它。
          ...
          <AccordionItem value="item-2">
            ...
            <AccordionContent>
              Zustand 是一个小巧、快速、可扩展的 React 状态管理库。
              它使用简洁的 API 来创建全局状态存储（Store），
              没有样板代码，不需要 Provider 包裹组件树，
              并且支持 TypeScript、中间件和 devtools。
            </AccordionContent>
          </AccordionItem>
...
export default function TechShowcase() {
  ...
  if (!mounted) {
    return <div className="space-y-8 not-prose min-h-[500px] flex items-center justify-center text-muted-foreground">Loading components...</div>
  }
  ...
  return (
    <div className="space-y-8 not-prose">
      ...
      <div className="flex flex-wrap gap-2 justify-center p-4 rounded-xl bg-gradient-to-r from-primary/5 via-primary/10 to-primary/5 border border-primary/10">
        <Badge className="bg-blue-500 text-white border-none hover:bg-blue-600">React 19</Badge>
        <Badge className="bg-cyan-500 text-white border-none hover:bg-cyan-600">Tailwind CSS 3</Badge>
        <Badge className="bg-zinc-800 text-white border-none hover:bg-zinc-900 dark:bg-zinc-200 dark:text-zinc-800">shadcn/ui</Badge>
        <Badge className="bg-amber-500 text-white border-none hover:bg-amber-600">Zustand 5</Badge>
        <Badge className="bg-purple-500 text-white border-none hover:bg-purple-600">Astro MDX</Badge>
      </div>
