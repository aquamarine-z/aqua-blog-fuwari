...
function TitleBarButton({ icon: Icon, onClick, title }: { icon: any, onClick: () => void, title?: string }) {
    return (
        <button 
            title={title}
            onClick={onClick} 
        ...
        <div className="w-full flex flex-col items-center justify-center py-2">
            {showTitleBar && (
                <div className="w-full flex justify-end gap-2 mb-2">
                    {playable && (
                        <TitleBarButton icon={Play} title="Play" onClick={() => {
                            if (svgRef.current) onPlay(svgRef.current, data);
                        }}/>
                    )}
                    {canMaximize && (
                        <TitleBarButton icon={Maximize2} title="Maximize" onClick={() => {
                            showMaximumImage(<AdaptiveSVG {...props} canMaximize={false} />);
                        }}/>
                    )}
                </div>
            )}
