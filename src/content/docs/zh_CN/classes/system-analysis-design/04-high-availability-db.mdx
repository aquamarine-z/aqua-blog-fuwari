...
**状态清单**：`CREATED`（已创建）、`PROCESSING`（处理中）、`SUCCESS`（交易成功）、`FAILED`（交易失败）。
...
#### 图例 B：信用卡申请与卡片生命周期状态转换图（CardApplicationStatus） [PDF 1 - P17]

**状态清单**：`CREATED`（已创建）、`REVIEWING`（审批中）、`ACTIVE`（已激活发卡）、`REJECTED`（已驳回）、`LOST`（挂失冻结）、`CLOSED`（已销户）。
