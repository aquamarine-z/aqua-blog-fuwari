...
*   **回归测试**：在代码修改（修复 Bug、系统增强、配置改变）之后进行测试，以确保旧有功能未受到隐式影响而退化。
    *   **回归测试常用策略**：
        1.  **再测试所有用例 (Retest All)**：重新执行原先所有的测试用例。最安全，但时间与资源成本最高，适用于关键核心版本。
        2.  **基于选择的选择性测试 (Selective Testing)**：利用特定准则只挑选一部分用例运行。包括：基于受影响的模块/功能点选择、基于高风险业务链路/核心功能点选择、基于近期发生缺陷的统计热点选择。
