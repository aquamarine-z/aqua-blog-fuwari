		...
		<VisualizationDisplay
			...
			onInitialize={(svg) => {
				...
				d3.select(svg)
					.append("g")
					.attr("class", "xAxis")
					.attr("transform", `translate(0, ${yScale(0) + 100})`)
					.call(xAxis);
				d3.select(svg)
					.append("g")
					.attr("class", "yAxis")
				...
				d3.select(svg)
					...
					.attr("x", padding - 50)
					.attr("y", padding - 50)
					.attr("width", width - 2 * padding + 100)
					.attr("height", height - 2 * padding + 150)
					.attr("fill", "none")
					.attr("stroke", "black")
					.attr("stroke-width", 2);
	...
	return (
		<VisualizationDisplay
			...
			onInitialize={(svg) => {
				...
				const yAxis = d3
					.axisLeft(yScale)
					.tickValues(d3.range(0.02, 0.2, 0.02))
				...
				const drawBox = (index: number, color: string) => {
					...
					const diagramGroup = d3
						.select(svg)
						.append("g")
						.attr("class", `box-${index}`);
					diagramGroup
						.append("line")
						.attr("class", "line-min")
	...
	return (
		<VisualizationDisplay
			...
			onInitialize={(svg) => {
				...
				d3.select(svg)
					.selectAll(".xAxisBottom text")
					.style("font-size", "24px")
					.style("transform", "translateY(10px)");
				d3.select(svg)
					.selectAll(".yAxisLeft text")
					.style("font-size", "24px")
					.style("transform", "translateX(-10px)");
				const hexagonGroup = d3
					.select(svg)
					.append("g")
				...
				const isPointInHexagon = (x, y, centerX, centerY, sideLength) => {
					...
					// 坐标系顺时针旋转 30 度
					const rotatedX =
						localX * Math.cos(Math.PI / 6) + localY * Math.sin(Math.PI / 6);
					const rotatedY =
						-localX * Math.sin(Math.PI / 6) + localY * Math.cos(Math.PI / 6);
					...
					return (
						absX <= sideLength &&
						absY <= (sideLength * Math.sqrt(3)) / 2 &&
						absY <= -absX * Math.sqrt(3) + sideLength * Math.sqrt(3)
					);
				};
				const drawHexagon = (
					x: number,
					...
					for (let i = 0; i < 6; i++) {
						...
						points.push(
							`${(x + r * Math.cos(angle)).toFixed(2)},${(y + r * Math.sin(angle)).toFixed(2)}`,
						);
					}
				...
				for (let i = 3.2; i < 5.8; i += 1.5 * r) {
					...
					for (let j = 3.2; j < 5.8; j += Math.sqrt(3) * r) {
						numbers.push(
							data.filter((d) =>
								isPointInHexagon(d.x, d.y, j + offsetValue, i, r),
							).length,
						);
					}
					offset = !offset;
				...
				for (let i = 3.2; i < 5.8; i += 1.5 * r) {
					...
					for (let j = 3.2; j < 5.8; j += Math.sqrt(3) * r) {
						...
						drawHexagon(
							xScale(j + offsetValue),
							yScale(i),
							xScale(3 + r) - xScale(3),
							getGradientColor(count / d3.max(numbers)),
						);
					}
					offset = !offset;
				}

				const xValueGroup = d3
					.select(svg)
					.append("g")
					.attr("class", "x-value-group");
				const gap = 0.05;
				const delta = 0.3;
				const xNumbers = [];
				...
				for (let i = 3 + delta / 2; i < 6; i += delta) {
					...
					xValueGroup
						.append("rect")
						.attr("x", xScale(i) - halfWidth)
						.attr("y", padding - xValueScale(count))
						.attr("width", 2 * halfWidth)
						.attr("height", xValueScale(count))
						.attr("fill", "blue")
						.attr("stroke", "black")
						.attr("stroke-width", "3");
				}

				const yValueGroup = d3
					.select(svg)
					.append("g")
					.attr("class", "y-value-group");
				const yNumbers = [];
				for (let i = 3 + delta / 2; i < 6; i += delta) {
				...
				for (let i = 3 + delta / 2; i < 6; i += delta) {
					...
					yValueGroup
						...
						.attr("y", yScale(i) - halfWidth)
						.attr("height", 2 * halfWidth)
						.attr("width", yValueScale(count))
						.attr("fill", "blue")
						.attr("stroke", "black")
						.attr("stroke-width", "3");
				}
...
export const Diagram6 = () => {
	...
	for (let i = 0; i < 15; i++) {
		const array = [];
		for (let j = 0; j < 15; j++) {
			array.push(-Number.parseFloat(Math.random().toFixed(1)));
		}
		data.push(array);
	}
	console.log(data);
			...
			onInitialize={(svg) => {
				...
				d3.select(svg)
					...
					.attr(
						"transform",
						(d) =>
							`rotate(-90,${xScale(d) + edge / 2},${height - padding + 100})`,
					);
				d3.select(svg)
					.append("g")
					.attr("class", "yAxis")
				...
				data.forEach((row, i) => {
					row.forEach((value, j) => {
						d3.select(svg)
							.append("rect")
							.attr("x", xScale(dataNames[j]))
							.attr("y", yScale(dataNames[i]))
							.attr("width", edge)
							.attr("height", edge)
							.attr(
								"fill",
								`rgb(${Math.abs(value) * 255},${Math.abs(value) * 255},255)`,
							)
							.attr("stroke", "white")
							.attr("stroke-width", "3");
						d3.select(svg)
							.append("text")
							.text(value)
							.attr("x", xScale(dataNames[j]) + edge / 2)
							.attr("y", yScale(dataNames[i]) + edge / 2 + 12.5)
							.attr("text-anchor", "middle")
							.attr("font-size", "25px")
							.attr("fill", "black");
					});
