	...
	return (
		<VisualizationDisplay
			...
			onInitialize={(svg) => {
				...
				circle.attr("cx", 30).attr("cy", 30).attr("r", 20).attr("fill", "red");
			}}
		>
			{children}
		</VisualizationDisplay>
	...
	return (
		<VisualizationDisplay
			...
			onInitialize={(svg) => {
				...
				path
					.attr("d", "M100 100 L200 200 L300 100 L400 400")
					.attr("stroke", "blue")
					.attr("stroke-width", 5)
					.attr("fill", "none");
			}}
	...
	return (
		<VisualizationDisplay
			...
			onInitialize={(svg) => {
				d3.select(svg)
					.attr("width", 600)
					.attr("height", 400) //设置svg的属性为宽600高400
		...
		<VisualizationDisplay
			...
			onInitialize={(svg) => {
				d3.select(svg)
					.attr("width", 600)
					.attr("height", 400)
					.attr("style", "border:black solid 2px;border-radius:5px;");
		...
		<VisualizationDisplay
			...
			onInitialize={(svg) => {
				...
				d3.select(svg)
					...
					.attr("class", "point")
					.attr("cx", (_, i) => xScale(i))
					.attr("cy", (d) => yScale(d)) //设置点的x y坐标
					.attr("r", 4)
					.attr("fill", "red"); //设置点的半径为4 颜色为红色
				d3.select(svg)
					.selectAll("data-text")
					.data(data)
					.enter()
					...
					.attr("x", (_, i) => xScale(i))
					.attr("y", (d) => yScale(d) - 10) //设置各个数据文字的坐标 注意yScale(d)-10的作用是让数据文字出现在点的上方
					.text((d) => d);
			}}
		>
			{children}
