...
// 1. 扫描原有通用语言文件
const globalTranslations = import.meta.glob<{ [key: string]: Translation }>('./languages/*.ts', { eager: true });
// 2. 扫描所有部分(partials)的语言文件 (例如 ./partials/js-playground/zh_CN.ts)
const partialTranslations = import.meta.glob<{ [key: string]: Translation }>('./partials/**/*.ts', { eager: true });

const map: { [key: string]: Translation } = {};

function mergeTranslation(path: string, moduleObj: any, langName: string, expectedPrefix?: string) {
    ...
    if (translation) {
        ...
        for (const key in translation) {
            if (expectedPrefix) {
                ...
                if (/^\[.*?\]/.test(key)) {
                    throw new Error(`Global translation key '${key}' in '${path}' cannot start with square brackets '[...]'`);
                }
            }
...
for (const path in partialTranslations) {
    ...
    if (folderMatch) {
        ...
        mergeTranslation(path, partialTranslations[path], langName, `[${folderName}]`);
    }
}
